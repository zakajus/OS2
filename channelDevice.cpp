...
void ChannelDevice::copyFromExternalMemory(uint32_t* dest) {
    uint32_t* hddMemoryPtr = hardDisk->getMemoryPointer();

    uint8_t buffer[rnum];
    uint8_t temp[4];
    uint32_t word;
    uint32_t target =  ('*' << 24) | ('*' << 16) | ('*' << 8) | '*';

    uint32_t currentWordIndex = 0;

    while(1){
        if (currentWordIndex == HDD_SIZE) {
            
        }
        word = hardDisk->getWord(currentWordIndex);
    ...
    vector<uint8_t> data = hardDisk->readBlock(currentWordIndex, rnum);
     if (data.size() < rnum) {
        //nepakankamai duomenu nukopijuota
        //realMachine->changePI();
    }
