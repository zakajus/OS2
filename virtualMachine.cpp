...
VirtualMachine::VirtualMachine(uint32_t &rax, uint32_t &rbx, uint16_t &ds, uint16_t &cs, uint16_t &pc, StatusFlag &sf,
...
void VirtualMachine::add(uint8_t x, uint8_t y) {
    // suziuret ar cia viskas logiska
    ...
    *rax += valueToAdd;
    ...
    // Check for unsigned overflow (carry flag)// If there was overflow, the result will be smaller than original value
    sf->zf = (*rax == 0) ? 1 : 0; // Check zero flag
    sf->af = (((originalRax & 0x0F) + (valueToAdd & 0x0F)) > 0x0F) ? 1 : 0;
    // Check auxiliary flag (carry from bit 3 to bit 4)
...
void VirtualMachine::multiply(uint8_t x, uint8_t y) {
    //istestuot paziuret kur tas musu atsakymas galu gale gaunasi
    int realAddress = realMachine->translateLocalAdressToRealAddress(x, y);
...
void VirtualMachine::saveFromAX(uint8_t x, uint8_t y) {
    //patikrint
    int realAddress = realMachine->translateLocalAdressToRealAddress(x, y);
    realMachine->saveWordToMemoryFromAx(realAddress);
}

void VirtualMachine::saveFromBX(uint8_t x, uint8_t y) {
    //patikrint
    int realAddress = realMachine->translateLocalAdressToRealAddress(x, y);
