...
void RealMachine::testavimui() {
    ...
    for (int i = 0; i < 16; ++i) {
        //cia galimai reiks pakeist kad nebekopijuotu ten kkur nieko nebera
...
int RealMachine::convertTextToProgram() {
    ...
    for (int i = 1; i < 256; ++i) {
        ...
        if (isValidHexDigit(word[0]) && isValidHexDigit(word[1]) && isValidHexDigit(word[2]) &&
            ...
            //nuo 33 iki 126 gali but characteriai kur tiesiog tekstines kontantos
            int o = 0;
            for (int j = 0; j < 4; ++j) {
...
void RealMachine::printAllRegisterValues() {
    cout << "RAX: " << rax << " RBX: " << rbx << " MODE: " << mode << " DS: " << ds << " CS: " << cs << " PC: " << pc <<
            " TI: " << ti << " DI: " << pi << " SI: " << si << " PTR: " << ptr << endl;
...
int RealMachine::test_() {
    ...
    if (pi > 0) {
        switch (pi) {
            //galima arba ignoruot arba isjungt viska lauk ir pritnit viska kas ne taip
            case 1: {
